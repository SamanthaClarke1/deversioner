...
if [ "$1" == "--glbl" ] || [ "$2" == "--glbl" ] || [ "$3" == "--glbl" ] || [ "$4" == "--glbl" ] || [ "$5" == "--glbl" ] || [ "$6" == "--glbl" ] || [ "$7" == "--glbl" ]; then
	GLBL=true
fi
if [ "$1" == "--safe" ] || [ "$2" == "--safe" ] || [ "$3" == "--safe" ] || [ "$4" == "--safe" ] || [ "$5" == "--safe" ] || [ "$6" == "--safe" ] || [ "$7" == "--safe" ]; then
	SAFE=true
fi
if [ "$1" == "--help" ] || [ "$2" == "--help" ] || [ "$3" == "--help" ] || [ "$4" == "--help" ] || [ "$5" == "--help" ] || [ "$6" == "--help" ] || [ "$7" == "--help" ]; then
	...
	echo "RootPath=$ROOTPATH MaxVers=$MAXVERS Safe=$SAFE Glbl=$GLBL"
	if [ $SAFE == false ]; then
	...
	clean_folder() {
		...
		while [ $(($(echo ${vers} | tr ' ' '\n' | wc -l)-$safevers)) -gt $MAXVERS ]; do	
			...
			printf -v ni "%03d" $i
			if [ $SAFE == true ]; then		
				echo -e "\e[33m (would be) removing $1/*v${ni}*\e[31m"
				echo "(would've) removed $1/*v${ni}*" | cat >> "$DATEE.log"
			else
